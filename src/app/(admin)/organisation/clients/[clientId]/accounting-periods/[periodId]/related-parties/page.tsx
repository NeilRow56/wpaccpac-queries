...
import {
  getRelatedPartiesScheduleAction,
  saveRelatedPartiesScheduleAction
} from '@/server-actions/simple-schedules/related-parties'

const CODE = 'B61-related_parties'

const TEMPLATE_ATTACHMENT_IDS = ['related-parties-detail'] as const
...
export default async function RelatedPartiesPage({
  ...
  const crumbs = buildPeriodLeafBreadcrumbs({
    ...
    leafLabel: 'Related Parties',
    leafHref: `/organisation/clients/${clientId}/accounting-periods/${periodId}/related-parties`
  })
        ...
        <div className='flex items-start gap-3'>
          ...
          <div>
            ...
            <p className='text-muted-foreground text-sm'>
              You must have the selected accounting period open to edit related
              parties.
            </p>
          </div>
        </div>
  ...
  const res = await getRelatedPartiesScheduleAction({ clientId, periodId })
  ...
  return (
    <div className='container mx-auto space-y-6 py-10'>
      ...
      <div className='flex items-center justify-between gap-3'>
        ...
        <div className='flex flex-col items-start gap-2 pr-2 xl:flex-row'>
          ...
          <Button asChild variant='outline' size='sm' className='text-blue-600'>
            <Link
              href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/planning/B61-related_parties_wp`}
            >
              B61 Related parties work programme
            </Link>
          </Button>
        </div>
      </div>
      ...
      <SimpleScheduleForm
        ...
        onSave={saveRelatedPartiesScheduleAction}
        derivedLineIds={[]}
        derivedHelpByLineId={{}}
