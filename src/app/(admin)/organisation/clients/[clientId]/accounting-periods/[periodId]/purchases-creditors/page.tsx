...
import {
  getCreditorsScheduleAction,
  saveCreditorsScheduleAction
} from '@/server-actions/simple-schedules/creditors'
...
const DERIVED_LINE_IDS = [
  ...
  ACCRUALS_DEFERRED_INCOME_LINE_ID
] as const
...
> = {
  ...
  [VAT_OTHER_TAXES_LINE_ID]: 'Derived from VAT and other taxes schedule.',
  [ACCRUALS_DEFERRED_INCOME_LINE_ID]:
    'Derived from Accruals and deferred income schedule.'
}
...
export default async function CreditorsPage({
  ...
  const res = await getCreditorsScheduleAction({ clientId, periodId })
  ...
  return (
    <div className='container mx-auto space-y-6 py-10'>
      ...
      <div className='flex items-center justify-between gap-3'>
        <span>
          <h1 className='text-primary text-lg font-semibold'>Creditors</h1>
        </span>
        ...
        <div className='flex flex-col items-start gap-2 pr-2 xl:flex-row'>
          ...
          <Button asChild variant='outline' size='sm' className='text-blue-600'>
            <Link
              href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/planning/B61-creditors_wp`}
            >
              B61 Creditors work programme
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className='flex flex-col gap-2 sm:flex-row'>
        <Button asChild variant='outline' size='sm'>
          <Link
            href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/purchases-creditors/trade-creditors`}
          >
        ...
        <Button asChild variant='outline' size='sm'>
          <Link
            href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/purchases-creditors/vat-and-other-taxes`}
          >
            VAT and other taxes
          </Link>
        </Button>
        <Button asChild variant='outline' size='sm'>
          <Link
            href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/purchases-creditors/accruals-and-deferred-income`}
          >
            Accruals schedule
          </Link>
        </Button>
      ...
      <SimpleScheduleForm
        ...
        onSave={saveCreditorsScheduleAction}
        // ✅ lock derived inputs in the UI
