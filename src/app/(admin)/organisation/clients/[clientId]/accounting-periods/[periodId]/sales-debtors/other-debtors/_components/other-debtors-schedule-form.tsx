...
import { saveOtherDebtorsScheduleAction } from '@/server-actions/schedules/other-debtors'
...
export default function OtherDebtorsScheduleForm({
  clientId,
  periodId,
  initial,
  prior,
  ...
  async function onSubmit(values: LineItemScheduleDocV1) {
    const res = await saveOtherDebtorsScheduleAction({
      clientId,
      periodId,
      doc: values
    })
    if (res.success) {
      toast.success('Other debtors saved')
      form.reset(values) // ✅ marks clean
    } else {
      toast.error(res.message ?? 'Failed to save other debtors')
    }
  }
