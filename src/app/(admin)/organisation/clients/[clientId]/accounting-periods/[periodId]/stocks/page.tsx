...
import {
  getStocksScheduleAction,
  saveStocksScheduleAction
} from '@/server-actions/simple-schedules/stocks'
...
export default async function StocksPage({
  ...
  const crumbs = buildPeriodLeafBreadcrumbs({
    ...
    leafLabel: 'Stocks',
    leafHref: `/organisation/clients/${clientId}/accounting-periods/${periodId}/stocks`
  })
        ...
        <div className='flex items-start gap-3'>
          ...
          <div>
            ...
            <p className='text-muted-foreground text-sm'>
              You must have the selected accounting period open to edit stocks.
            </p>
          </div>
        </div>
      </div>
  ...
  const res = await getStocksScheduleAction({ clientId, periodId })
  ...
  return (
    <div className='container mx-auto space-y-6 py-10'>
      ...
      <div className='flex items-center justify-between gap-3'>
        <span>
          <h1 className='text-primary text-lg font-semibold'>Stocks</h1>
        </span>
        ...
        <div className='flex flex-col items-start gap-2 pr-2 xl:flex-row'>
          ...
          <Button asChild variant='outline' size='sm' className='text-blue-600'>
            <Link
              href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/planning/B61-stocks_wp`}
            >
              B61 Stocks work programme
            </Link>
          </Button>
        </div>
      </div>
      ...
      <SimpleScheduleForm
        ...
        onSave={saveStocksScheduleAction}
        derivedLineIds={[]}
        derivedHelpByLineId={{}}
