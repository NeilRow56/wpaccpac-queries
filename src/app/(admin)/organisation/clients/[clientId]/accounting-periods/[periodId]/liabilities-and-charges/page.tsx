...
import {
  getLiabilitiesAndChargesScheduleAction,
  saveLiabilitiesAndChargesScheduleAction
} from '@/server-actions/simple-schedules/liabilities-and-charges'

const CODE = 'B61-liabilities_and_charges'

const TEMPLATE_ATTACHMENT_IDS = ['tax-transfer', 'other-support'] as const

const DERIVED_LINE_IDS = [
  'bf-balance',
  'tax-movement',
  'tax-period-end-balance'
] as const

export default async function LiabilitiesAndChargesPage({
  params
}: {
  ...
  const crumbs = buildPeriodLeafBreadcrumbs({
    ...
    leafLabel: 'Liabilities and charges',
    leafHref: `/organisation/clients/${clientId}/accounting-periods/${periodId}/liabilities-and-charges`
  })
        ...
        <div className='flex items-start gap-3'>
          ...
          <div>
            ...
            <p className='text-muted-foreground text-sm'>
              You must have the selected accounting period open to edit this
              schedule.
            </p>
          </div>
        </div>
  ...
  const res = await getLiabilitiesAndChargesScheduleAction({
    clientId,
    periodId
  })
  ...
  return (
    <div className='container mx-auto space-y-6 py-10'>
      ...
      <div className='flex items-center justify-between gap-3'>
        ...
        <div className='flex flex-col items-start gap-2 pr-2 xl:flex-row'>
          ...
          <Button asChild variant='outline' size='sm' className='text-blue-600'>
            <Link
              href={`/organisation/clients/${clientId}/accounting-periods/${periodId}/planning/B61-liabilities_and_charges_wp`}
            >
              B61 Liabilities and charges work programme
            </Link>
          </Button>
        </div>
      </div>
      ...
      <SimpleScheduleForm
        title='Liabilities and charges'
        clientId={clientId}
        periodId={periodId}
        initial={res.data.current}
        prior={res.data.prior}
        priorPeriod={priorPeriod}
        onSave={saveLiabilitiesAndChargesScheduleAction}
        derivedLineIds={[...DERIVED_LINE_IDS]}
        derivedHelpByLineId={{
          'bf-balance': 'Auto-seeded from prior period closing balance.',
          'tax-movement': 'Auto-transferred from Taxation schedule.',
          'tax-period-end-balance': 'Auto-transferred from Taxation schedule.'
        }}
