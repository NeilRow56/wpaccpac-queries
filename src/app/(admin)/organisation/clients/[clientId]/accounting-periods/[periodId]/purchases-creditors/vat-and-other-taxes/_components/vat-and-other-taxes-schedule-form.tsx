...
import { saveVatAndOtherTaxesScheduleAction } from '@/server-actions/schedules/vat-and-other-taxes'
...
export default function VatAndOtherTaxesScheduleForm({
  clientId,
  periodId,
  initial,
  prior,
  ...
  async function onSubmit(values: LineItemScheduleDocV1) {
    const res = await saveVatAndOtherTaxesScheduleAction({
      clientId,
      periodId,
      doc: values
    })
    if (res.success) {
      toast.success('VAT and other taxes saved')
      form.reset(values) // ✅ marks clean
    } else {
      toast.error(res.message ?? 'Failed to save VAT and other taxes')
    }
  }
