// server-actions/simple-schedules/trial-balance-and-journals.ts
'use server'
...
import { trialBalanceAndJournalsDefault } from '@/lib/schedules/templates/trial-balance-and-journals'
import type {
  SimpleScheduleDocV1,
  ScheduleLine,
  ScheduleLineUi,
...
function ensureTrialBalanceAndJournalsAttachments(doc: SimpleScheduleDocV1): {
  ...
  ensure('trial-balance', 'Trial balance')
  ensure('journals', 'Journals')
...
export async function getTrialBalanceAndJournalsScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    let current: SimpleScheduleDocV1 =
      normalized ?? trialBalanceAndJournalsDefault

    const ensuredAttachments = ensureTrialBalanceAndJournalsAttachments(current)
    current = ensuredAttachments.doc

    if (existing && ensuredAttachments.changed) {
      await db
        .update(planningDocs)
    ...
    if (prior) {
      ...
      if (priorDoc) {
        priorDoc = ensureTrialBalanceAndJournalsAttachments(priorDoc).doc
      }
    }
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to load trial balance and journals'
    }
  }
}

export async function saveTrialBalanceAndJournalsScheduleAction(input: {
  clientId: string
  periodId: string
  doc: SimpleScheduleDocV1
  ...
  try {
    ...
    const ensuredAttachments = ensureTrialBalanceAndJournalsAttachments(
      input.doc
    )
    const docToSave = ensuredAttachments.doc
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/trial-balance-and-journals`
    )
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to save trial balance and journals'
    }
  }
}
