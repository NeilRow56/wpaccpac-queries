...
import { wagesAndSalariesDefault } from '@/lib/schedules/templates/wages-and-salaries'
import type {
  SimpleScheduleDocV1,
  ScheduleLine,
  ScheduleLineUi,
...
function ensureWagesAttachments(doc: SimpleScheduleDocV1): {
  ...
  ensure('payroll-summary', 'Payroll summary')
  ensure('paye-nic-control', 'PAYE and NIC control')
...
export async function getWagesAndSalariesScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    let current: SimpleScheduleDocV1 = normalized ?? wagesAndSalariesDefault

    const ensuredAttachments = ensureWagesAttachments(current)
    ...
    if (prior) {
      ...
      if (priorDoc) {
        priorDoc = ensureWagesAttachments(priorDoc).doc
      }
    }
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to load wages and salaries'
    }
  }
}

export async function saveWagesAndSalariesScheduleAction(input: {
  clientId: string
  periodId: string
  doc: SimpleScheduleDocV1
  ...
  try {
    ...
    const ensuredAttachments = ensureWagesAttachments(input.doc)
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/wages-and-salaries`
    )
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to save wages and salaries'
    }
  }
}
