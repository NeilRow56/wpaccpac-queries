// server-actions/simple-schedules/vat-summary.ts
'use server'
...
import { vatSummaryDefault } from '@/lib/schedules/templates/vat-summary'
import type {
  SimpleScheduleDocV1,
  ScheduleLine,
  ScheduleLineUi,
...
function ensureVatSummaryAttachments(doc: SimpleScheduleDocV1): {
  ...
  ensure('vat-summary', 'VAT summary')
...
export async function getVatSummaryScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    let current: SimpleScheduleDocV1 = normalized ?? vatSummaryDefault

    const ensuredAttachments = ensureVatSummaryAttachments(current)
    ...
    if (prior) {
      ...
      if (priorDoc) {
        priorDoc = ensureVatSummaryAttachments(priorDoc).doc
      }
    }
    ...
    return {
      success: false,
      message: e instanceof Error ? e.message : 'Failed to load VAT summary'
    }
  }
}

export async function saveVatSummaryScheduleAction(input: {
  clientId: string
  periodId: string
  doc: SimpleScheduleDocV1
  ...
  try {
    ...
    const ensuredAttachments = ensureVatSummaryAttachments(input.doc)
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/vat-summary`
    )
    ...
    return {
      success: false,
      message: e instanceof Error ? e.message : 'Failed to save VAT summary'
    }
  }
}
