...
import { relatedPartiesDefault } from '@/lib/schedules/templates/related-parties'
...
export async function getRelatedPartiesScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    const current: SimpleScheduleDocV1 = normalized ?? relatedPartiesDefault
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to load related parties schedule'
    }
  }
}

export async function saveRelatedPartiesScheduleAction(input: {
  clientId: string
  periodId: string
  ...
  try {
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/related-parties`
    )
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to save related parties schedule'
    }
  }
}
