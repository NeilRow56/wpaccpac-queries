// server-actions/simple-schedules/sundry-workings.ts
'use server'
...
import { sundryWorkingsDefault } from '@/lib/schedules/templates/sundry-workings'
import type {
  SimpleScheduleDocV1,
  ScheduleLine,
  ScheduleLineUi,
...
function ensureSundryWorkingsAttachments(doc: SimpleScheduleDocV1): {
  ...
  ensure('drawings-summary', 'Drawings summary')
...
export async function getSundryWorkingsScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    let current: SimpleScheduleDocV1 = normalized ?? sundryWorkingsDefault

    const ensuredAttachments = ensureSundryWorkingsAttachments(current)
    ...
    if (prior) {
      ...
      if (priorDoc) {
        priorDoc = ensureSundryWorkingsAttachments(priorDoc).doc
      }
    }
    ...
    return {
      success: false,
      message: e instanceof Error ? e.message : 'Failed to load sundry workings'
    }
  }
}

export async function saveSundryWorkingsScheduleAction(input: {
  clientId: string
  periodId: string
  doc: SimpleScheduleDocV1
  ...
  try {
    ...
    const ensuredAttachments = ensureSundryWorkingsAttachments(input.doc)
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/sundry-workings`
    )
    ...
    return {
      success: false,
      message: e instanceof Error ? e.message : 'Failed to save sundry workings'
    }
  }
}
