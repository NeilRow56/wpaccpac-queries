...
import { cashAtBankAccountsDefault } from '@/lib/schedules/templates/cash-at-bank-accounts'

const CODE = 'B61-cash_at_bank_accounts'
...
export async function getCashAtBankAccountsScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        totals: {
          ...
          prior: sum(priorDoc?.rows ?? [], 'current')
        },
        priorPeriod: prior
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to load cash at bank accounts schedule'
    }
  }
}

export async function saveCashAtBankAccountsScheduleAction(input: {
  clientId: string
  periodId: string
  ...
  try {
    ...
    // ✅ Revalidate both lead + supporting page
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/cash-at-bank`
    )
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/cash-at-bank/bank-accounts`
    )
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to save cash at bank accounts schedule'
    }
  }
}
