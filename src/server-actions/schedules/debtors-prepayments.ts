...
import { debtorsPrepaymentsDefault } from '@/lib/schedules/templates/debtors-prepayments'
import type {
  LineItemScheduleDocV1,
  LineItemScheduleRowV1
} from '@/lib/schedules/lineItemScheduleTypes'

const CODE = 'B61-debtors_prepayments'
...
> {
  try {
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to load prepayments schedule'
    }
  }
}

export async function saveDebtorsPrepaymentsScheduleAction(input: {
  clientId: string
  periodId: string
  ...
  try {
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/sales-debtors/prepayments`
    )
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to save prepayments schedule'
    }
  }
}
