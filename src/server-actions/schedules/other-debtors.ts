...
import { otherDebtorsDefault } from '@/lib/schedules/templates/other-debtors'
...
export async function getOtherDebtorsScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
...
> {
  try {
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to load other debtors schedule'
    }
  }
}

export async function saveOtherDebtorsScheduleAction(input: {
  clientId: string
  periodId: string
  ...
  try {
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/sales-debtors/other-debtors`
    )
    ...
    return {
      success: false,
      message:
        e instanceof Error ? e.message : 'Failed to save other debtors schedule'
    }
  }
}
