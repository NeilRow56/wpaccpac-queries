...
import { tradeCreditorsDefault } from '@/lib/schedules/templates/trade-creditors'
import type {
...
const CODE = 'B61-trade_creditors'
...
export async function getTradeCreditorsScheduleAction(input: {
  clientId: string
  periodId: string
}): Promise<
  ActionResult<{
  ...
  try {
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to load trade creditors schedule'
    }
  }
}

export async function saveTradeCreditorsScheduleAction(input: {
  clientId: string
  periodId: string
  ...
  try {
    ...
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/sales-creditors`
    )
    revalidatePath(
      `/organisation/clients/${clientId}/accounting-periods/${periodId}/sales-creditors/trade-creditors`
    )
    ...
    return {
      ...
      message:
        e instanceof Error
          ? e.message
          : 'Failed to save trade creditors schedule'
    }
  }
}
